...
async function main() {
  ...
  for (const alert of alerts) {
    ...
    if (alert.alerteGarantieId && alert.garantie) {
      ...
      if (alertOwnerId !== warrantyOwnerId) {
        console.log(
          `❌ Alert ${alert.alerteId} owned by user ${alertOwnerId} but warranty ${alert.alerteGarantieId} owned by user ${warrantyOwnerId}`
        );
        incorrectAlerts++;
      }
    }
    ...
    if (alert.alerteArticleId && alert.article) {
      ...
      if (alertOwnerId !== articleOwnerId) {
        console.log(
          `❌ Alert ${alert.alerteId} owned by user ${alertOwnerId} but article ${alert.alerteArticleId} owned by user ${articleOwnerId}`
        );
        incorrectAlerts++;
      }
    }
  ...
  if (incorrectAlerts === 0) {
    console.log(
      "✅ All alerts are correctly owned by the same user as their associated articles/warranties"
    );
  } else {
  ...
  const orphanedAlerts = alerts.filter(
    (alert) => !alert.alerteGarantieId && !alert.alerteArticleId
  );
  console.log(
    `Alerts with no associated warranty or article: ${orphanedAlerts.length}`
  );
  ...
  if (orphanedAlerts.length > 0) {
    ...
    orphanedAlerts.forEach((alert) => {
      console.log(
        `- Alert ${alert.alerteId}: "${alert.alerteNom}" owned by user ${alert.ownerUserId}`
      );
    });
  }
}
